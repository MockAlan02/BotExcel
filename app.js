...
const flowPrincipal = bot
  ...
  .addAction({ capture: true }, async (ctx, { flowDynamic, fallBack }) => {
    ...
    var coincidencias = ctx.body.match(regexp);
    if (!coincidencias) {
...
const flowMenu = bot
  ...
  .addAnswer(
    "Te interesa alguno de los productos que tenemos disponibles?",
    { capture: true },
    ...
    `);
      ...
      const regexp = /no/gmi
      // Regexp.prototype.test() methods tests if the regexp matches the string
      // If there's any no on the check (either "NO_EXISTE" or "No existe."), it will send to the flowEmpty
      if (regexp.test(getCheck)) {
...
const flowEmpty = bot
  ...
  .addAnswer("Disculpa, no he entendido tu pedido. Selecciona algo en el menu y se especifico con tu pedido.", null, async (_, { gotoFlow }) => {
